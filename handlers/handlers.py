...
async def profiles_handler(request: web.Request) -> web.Response:
    # Idea was like this: when create users, profiles for them create too.
    # Then we can get, post, put, delete(user and profile) requests for users
    # For profiles we can do only get and put requests
    async with async_session_factory() as session:
        try:
            match request.method:
                case "GET":
